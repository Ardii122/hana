...
class HanafudaClient {
  ...
  async findOptimalGasPrice() {
    try {
      ...
      const opFee = BigInt(
        latestBlock.opFeePerGas || (await this.web3.eth.getGasPrice())
      );
      const currentGasPrice = BigInt(await this.web3.eth.getGasPrice());

      // Convert to Gwei for display only
      const opFeeGwei = parseFloat(
        this.web3.utils.fromWei(opFee.toString(), "gwei")
      );
      ...
      logger.info(
        this.colors.style(
          `Network Optimism Fee: ${opFeeGwei.toFixed(2)} Gwei, ` +
  ...
  formatTxUrl(txHash) {
    ...
    const opUrl = this.selectedNetwork.EXPLORER_URL;
    const fullUrl = `${opUrl}/tx/0x${cleanHash}#eventlog`;
        ...
        choices: [
          {
            name: this.colors.style(
              `${NETWORKS.OPTIMISM.name} Network`,
              "menuOption"
            ),
